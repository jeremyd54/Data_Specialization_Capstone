...
These graphs show the most common two-word combinations and most of them will be fairly good predictors. In the blogs set, a number followed by "minutes" or two numbers are half of the top-20 further demonstrating that removing numbers could be a useful tools going forward.

## Building A Model

I plan to build a model on these word sources by first removing the non-English characters and perhaps numbers as well. Then, starting with bi-grams, I will use the first word as input and the second as a response. Using machine learning, I will fit models to see which has the best accuracy on a test sets. Then I will expand on the model with tri-grams and so on.
...
Since the data set were quite large, some operations took a very long time. With 16 Gb of RAM, retaining all the data objects caused the memory to overfill. Thus, the code contains saving data frames to the disc and removing them for memory. This also also them to be reloaded quickly in the future.
